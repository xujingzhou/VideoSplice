...
- (void)loadPhotos
{
    ...
    ALAssetsLibraryGroupsEnumerationResultsBlock listGroupBlock = ^(ALAssetsGroup *group, BOOL *stop) {
        ...
        if (_mediaType == OnlyPhotos)
        {
            onlyPhotosFilter = [ALAssetsFilter allPhotos];
        }
        else if (_mediaType == OnlyVideos)
        {
...
- (UIView *)topView
{
    if (_topView == nil)
    {
        ...
        titleLabel.text = GBLocalizedString(@"SELECT");
        ...
        rect = CGRectMake(CGRectGetWidth(navView.bounds)-100, 0, 100, CGRectGetHeight(navView.bounds));
        UIButton *cropBtn = [[UIButton alloc] initWithFrame:rect];
        [cropBtn setTitle:GBLocalizedString(@"OK") forState:UIControlStateNormal];
...
{
    // Check video length
    if (_videoMaxLength > 0)
    {
        CGFloat curVideoLength = getVideoDuration(self.cropView.alAsset.defaultRepresentation.url);
        if (curVideoLength > _videoMaxLength)
        {
            ...
            NSString *hint = [fileLenHint stringByAppendingFormat:@" %.0f ", _videoMaxLength];
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(IGAssetsPickerFinishCroppingToAsset:)])
    {
        ...
        [self.cropView setFinishBlock:^(BOOL success, id result){
            
            if (success)
            {
                NSLog(@"Crop Success: %@", result);
                ProgressBarDismissLoading(GBLocalizedString(@"Success"));
                
                dispatch_async(dispatch_get_main_queue(), ^{
                    
                    [weakSelf backAction];
                    [weakSelf.delegate IGAssetsPickerFinishCroppingToAsset:result];
                });
            }
            else
            {
        ...
        [self.cropView setExportProgressBlock: ^(NSNumber *percentage) {
            
            // Export progress
            [weakSelf retrievingProgress:percentage title:GBLocalizedString(@"SavingVideo")];
        }];
        
        [self.cropView cropAsset];
    }
